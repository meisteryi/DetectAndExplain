...
class GeminiService {
  ...
  Future<TranslationResult> analyzeSelectedText(String selectedText) async {
    final apiKey = dotenv.env[EnvKeys.geminiApiKey];
    if (apiKey == null ||
        apiKey.isEmpty ||
        apiKey == 'your_gemini_api_key_here') {
    ...
    final prompt =
        '너는 일본을 여행하는 한국인을 위한 현지 문화 가이드야. '
        '사용자가 선택한 일본어 텍스트 "$selectedText"를 상세 분석해줘. '
        '선택된 단어의 정확한 번역, 문화적 배경/해설, 주문 꿀팁, 그리고 해당 단어(메뉴)를 활용해서 현지 점원에게 말할 수 있는 완전한 일본어 주문용 예문 문장들을 다음 JSON 형식으로만 답변해줘.\n\n'
        '{\n'
